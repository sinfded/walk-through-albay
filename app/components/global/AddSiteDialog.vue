<template>
  <Dialog :modal="true" :open="siteModal">
    <form>
      <DialogTrigger as-child>
        <Button
          @click="siteModal = true"
          class="font-semibold"
          variant="default"
          >Add New</Button
        >
...
const openFilePicker = () => {
  ...
  else if (newSiteData.value.municipality_name == "")
    toast.info("Select a municipality first.");
  else if (newSiteData.value.name == "") toast.info("Fill up site name first.");
  else {
...
const handleFileChange = async (e: any) => {
  ...
  const fileName = `${newSiteData.value.municipality_name} ${newSiteData.value.name}-${selectedElevation.value}`;
  ...
  newSiteData.value.site_images[
    view as keyof typeof newSiteData.value.site_images
  ] = publicUrl;
};

const selectMunicipality = () => {
  const selectedMun = municipalities.find(
    (mun) => mun.id == newSiteData.value.municipality_id
  );
  if (selectedMun)
    newSiteData.value.municipality_name = selectedMun.properties.adm3_en;
};

const cleanCoordinates = (direction: string) => {
  if (direction == "latitude")
    newSiteData.value.coordinates.latitude = parseFloat(
      `${newSiteData.value.coordinates.latitude}`
    );
  else if (direction == "longitude")
    newSiteData.value.coordinates.longitude = parseFloat(
      `${newSiteData.value.coordinates.longitude}`
    );
};
